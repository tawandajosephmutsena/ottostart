                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                    {(items || []).map((pillar, i) => (
                        <AnimatedSection key={i} animation="fade-up" delay={i * 200}>
                            <div className="group h-full p-10 rounded-[40px] bg-white dark:bg-white/5 border border-black/5 dark:border-white/5 hover:bg-agency-accent dark:hover:bg-agency-accent transition-all duration-500 overflow-hidden relative">
                                <div className="text-5xl mb-8 group-hover:scale-110 transition-transform duration-500">{pillar.emoji}</div>
                                <h3 className="text-2xl font-black uppercase tracking-tighter text-agency-primary dark:text-white group-hover:text-agency-primary mb-4">{pillar.title}</h3>
                                <p className="text-agency-primary/60 dark:text-white/60 group-hover:text-agency-primary/80 font-medium leading-relaxed">
                                    {pillar.desc}
                                </p>
                            </div>
                        </AnimatedSection>
